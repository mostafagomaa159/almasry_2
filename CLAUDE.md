...
- **A guest cart also needs an email** (`setGuestEmailOnCart`), which the brief does not mention: without it `placeOrder` returns `code: "GUEST_EMAIL_MISSING"`. Nothing in the checkout design collects one, so it comes from the account — `PrefKeys.email`, written by email login and by the profile's email field. The shipping step sends it alongside the address and records `isEmailReady`, so a missing one is reported there rather than after the user has picked how to pay. Magento validates the format, so a malformed stored address is rejected too. **Known gap:** the phone/OTP login stores a phone and *clears* `PrefKeys.email`, so a phone-only user has to add an email in their profile before checking out. `RegisterCustomerOtpModel` already parses the OTP reply's `customer` object but drops everything except `id` — if that payload carries an email, persisting it there would close the gap.
